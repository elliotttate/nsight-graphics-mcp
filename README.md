...
in-app integration end-to-end â€” without the Nsight UI.
...
--help-all` â€” start triggers (`--start-after-frames` / `--start-after-ms`
/ `--start-after-hotkey` / `--start-with-ngfx-sdk` / â€¦), stop triggers
(`--limit-to-frames` / `--max-duration-ms` / `--stop-with-ngfx-sdk` / â€¦),
...
capture / trace hotkey in-app â€” the call returns a launch-session handle
you can poll with `ngfx_launch_status` and terminate with
`ngfx_launch_stop`.

### Headless capture (`ngfx-capture.exe`) â€” no Nsight UI required
...
### Object index (Pipelines / Shaders / Resources â€” GUI 'Resources' pane parity)
...
* **Inventory only (fast)** â€” string-scan for proto filenames and message
  names. No protobuf decoder required.
* **Full FileDescriptorProto extraction** â€” walks the embedded descriptor
...
| `ngfx_proto_search` | Regex-grep the inventory. E.g. `pattern='RootParam'` â†’ `PbRootParameter`, `PbRootDescriptor`, `PbRootParametersInfo`, ... |
...
| `ngfx_capture_functions` | Recorded function stream (`--metadata-functions`) â€” raw lines. |
...
| Tool | What it does |
| --- | --- |
| `ngfx_replay_run` | Replay a capture with full control: loop count, perf-report-dir, device selection (name / vendor / index), present mode, vsync, multibuffering, reset mode, â€¦ |
...
| `ngfx_replay_run_advanced` | Run `ngfx-replay` with arbitrary flags â€” surfaces ~65 hidden flags discovered by binary analysis (validation modes, NVAPI/NGX/NRC/DLSS toggles, force-* recovery modes, etc.). |
...
### Generate C++ Capture â†’ build â†’ run

The CLI activity (`ngfx --activity 'Generate C++ Capture'`) requires the
original application to launch â€” it can't run against a saved capture.
...
### Per-call argument extraction (C++ Capture â†’ SQLite index)

Nsight's CLI (`ngfx-replay --metadata-functions`) returns function NAMES
per event but **no argument values** â€” it's a deliberate design choice.
...
2. Click *File â†’ Activity â†’ Generate C++ Capture*, pick an output dir.
...
4. Per-event queries â€” full structured args for ~25 common
...
5. `ngfx_cpp_capture_descriptor_bindings` walks backwards from any
   ...
   CBV/SRV/UAV + VBV/IBV + RT state in effect at that event** â€” the
...
| Tool | What it does |
| --- | --- |
| `ngfx_top_n_costs` | Top-N rows by GPU time across all CSVs in a `ngfx-replay --perf-report-dir` output, or a `.nsight-gputrace` archive (auto-unzips). Sniffs column names (no hardcoded schema) and normalises units (ns/Âµs/ms/s). Filters: `kind_filter`, `name_regex`, `csv_basename_hint`. |
...
| `ngfx_sdk_snippet` | Codegen a ready-to-paste C++ snippet for `(activity, api)` â€” e.g. `(GPUTrace, Vulkan)` â†’ an init + start/stop pair. |
...
| Tool | What it does |
| --- | --- |
| `ngfx_glslang_compile` | Compile GLSL â†’ SPIR-V (uses bundled `glslang.exe`). |
...
* `mcp>=1.2` â€” MCP server SDK
* `pydantic>=2.0` â€” schema validation for MCP tool params
* `psutil>=5.9` â€” process / handle introspection for launch sessions
* `protobuf>=4.21` â€” decoding the embedded `FileDescriptorProto` blobs
...
`NSIGHT_GRAPHICS_MCP_NGFX_REPLAY`, â€¦).
...
finding the right install â€” pass the directory that contains
...
### 9. Escape hatch â€” anything not exposed by a typed tool
...
### 10. Per-call argument extraction (UI roundtrip â†’ SQLite index)
...
# â†’ {uid, type_name, category, create_call: {event_index, function_name},
#    mention_count, mentions_by_role: {bind: 14, write: 2, draw: 0, ...},
#    mentions: [...]}
```

### 12. Frame perf triage â€” top-N expensive draws
...
### 13. Capture diff â€” what changed between good and bad?
...
# â†’ function/kind histogram deltas + LCS-aligned insert/delete clusters,
...
- Some interactive UI features â€” the per-shader source profiler scroll,
  PSO browser, screenshot scrubber, frame-debugger pixel history â€” are
...
tests/                   # pytest suite â€” synthetic data for the parsers,
